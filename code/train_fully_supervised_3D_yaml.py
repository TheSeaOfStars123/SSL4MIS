...
from dataloaders.utils import _loader_classes
...
def train(config):
    ...
    iter_num = 0
    max_epoch = max_iterations // len(train_datasets) + 1
    ...
    for epoch_num in iterator:
        for t in loaders['train']:
            volume_batch, label_batch, weight = _split_training_batch(t)
            if len(list(label_batch.size())) == 2:
...
if __name__ == '__main__':
    ...
    shutil.copyfile(config['basic']['config'], snapshot_path + "/" + now + '.yaml')
    config['snapshot_path'] = snapshot_path
    # log experiment configuration
    logging.basicConfig(filename=snapshot_path + "/" + now + "_log.txt", level=logging.INFO,
